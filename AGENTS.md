...
### Prisma client
After pulling schema changes, run `pnpm db:generate` (or `pnpm install`, which runs it via postinstall). Stale generated clients cause TS errors on new models (`WaitlistSignup`, `TmcRingReview`, etc.).

### Lint / typed routes
`pnpm lint` runs ESLint directly (Next.js 16 removed `next lint`). `typedRoutes` is disabled in `next.config.mjs` because dynamic hrefs (e.g. `/shop?category=...`) fail strict `RouteImpl` checks.
